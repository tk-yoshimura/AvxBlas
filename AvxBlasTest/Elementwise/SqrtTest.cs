...
namespace AvxBlasTest.ElementwiseTest {
    ...
    public class SqrtTest {
        [TestMethod]
        public void SSqrtTest() {
            Random random = new(1234);

            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    float[] x = (new float[length]).Select((_, idx) => (float)random.Next(32)).ToArray();

                    float[] ts = (new float[length])
                        .Select((_, idx) => idx < count ? (float)Math.Sqrt(x[idx]) : 0)
                        .ToArray();

                    Array<float> y = new(length);

                    Elementwise.Sqrt(count, x, y);
            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    ...
                    double[] ts = (new double[length])
                        .Select((_, idx) => idx < count ? Math.Sqrt(x[idx]) : 0)
                        .ToArray();
