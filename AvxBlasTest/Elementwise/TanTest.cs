            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    ...
                    float[] ts = (new float[length])
                        .Select((_, idx) => idx < count ? (float)Math.Tan(x[idx]) : 0)
                        .ToArray();

                    Array<float> y = new(length);

                    Elementwise.Tan(count, x, y);

                    float[] ys = y;

                    for (int i = 0; i < ts.Length; i++) {
                        Assert.AreEqual(ts[i], ys[i], Math.Abs(ts[i]) * 1e-5);
                    }
                }
            }
        }
            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    ...
                    double[] ts = (new double[length])
                        .Select((_, idx) => idx < count ? Math.Tan(x[idx]) : 0)
                        .ToArray();
                    ...
                    for (int i = 0; i < ts.Length; i++) {
                        Assert.AreEqual(ts[i], ys[i], Math.Abs(ts[i]) * 1e-10);
                    }
                }
            }
        }
