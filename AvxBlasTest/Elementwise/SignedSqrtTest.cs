            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    ...
                    float[] ts = (new float[length])
                        .Select((_, idx) => idx < count ? (float)(Math.Sign(x[idx]) * Math.Sqrt(Math.Abs(x[idx]))) : 0)
                        .ToArray();

                    Array<float> y = new(length);

                    Elementwise.SignedSqrt(count, x, y);
            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    ...
                    double[] ts = (new double[length])
                        .Select((_, idx) => idx < count ? (Math.Sign(x[idx]) * Math.Sqrt(Math.Abs(x[idx]))) : 0)
                        .ToArray();
