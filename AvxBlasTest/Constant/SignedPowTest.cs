...
namespace AvxBlasTest.ConstantTest {
    ...
    public class SignedPowTest {
        ...
        public void SSignedPowTest() {
            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    float[] x = (new float[length]).Select((_, idx) => (float)random.Next(9) - 4).ToArray();

                    float[] t = (new float[length])
                        .Select((_, idx) => idx < count ? (float)(Math.Sign(x[idx]) * Math.Pow(Math.Abs(x[idx]), c)) : 0)
                        .ToArray();

                    Array<float> y = new(length);

                    Constant.SignedPow(count, x, c, y);
        ...
        public void DSignedPowTest() {
            ...
            for (uint length = 1; length <= 64; length++) {
                for (uint count = 0; count <= length; count++) {
                    double[] x = (new double[length]).Select((_, idx) => (double)random.Next(65) - 32).ToArray();

                    double[] t = (new double[length])
                        .Select((_, idx) => idx < count ? (Math.Sign(x[idx]) * Math.Pow(Math.Abs(x[idx]), c)) : 0)
                        .ToArray();
