...
namespace AvxBlasTest {
    public class Filter3D {
        ...
        public Filter3D(int inchannels, int kwidth, int kheight, int kdepth, int outchannels, float[] val = null) {
            if (kwidth < 1 || kheight < 1 || kdepth < 1 || inchannels < 1 || outchannels < 1) {
                throw new ArgumentException();
            }

            int length = checked(inchannels * kwidth * kheight * kdepth * outchannels);
        ...
        public Filter3D(int inchannels, int kwidth, int kheight, int kdepth, int outchannels, double[] val) {
            if (kwidth < 1 || kheight < 1 || kdepth < 1 || inchannels < 1 || outchannels < 1) {
                throw new ArgumentException();
            }
        ...
        public double this[int inch, int kx, int ky, int kz, int outch] {
            get {
                if (inch < 0 || inch >= InChannels || kx < 0 || kx >= KernelWidth ||
                    ky < 0 || ky >= KernelHeight || kz < 0 || kz >= KernelDepth ||
                    outch < 0 || outch >= OutChannels) {
                    throw new IndexOutOfRangeException();
                }

                return val[inch + InChannels * (kx + KernelWidth * (ky + KernelHeight * (kz + KernelDepth * outch)))];
            }
            set {
                if (inch < 0 || inch >= InChannels || kx < 0 || kx >= KernelWidth ||
                    ky < 0 || ky >= KernelHeight || kz < 0 || kz >= KernelDepth ||
                ...
                val[inch + InChannels * (kx + KernelWidth * (ky + KernelHeight * (kz + KernelDepth * outch)))] = value;
            }
        }
