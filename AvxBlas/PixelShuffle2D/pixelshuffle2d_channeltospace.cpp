...
int pixelshuffle2d_channeltospace_aligned(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint iy = 0; iy < ih; iy++) {
            for (uint cy = 0; cy < s; cy++) {
                const float* xc_ptr = x_ptr + cs * cy + ic * iw * iy;
...
int pixelshuffle2d_channeltospace_unaligned(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
...
int pixelshuffle2d_channeltospace_cs2to3(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
...
int pixelshuffle2d_channeltospace_cs4(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
...
int pixelshuffle2d_channeltospace_cs5to7(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
...
int pixelshuffle2d_channeltospace_cs8(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
...
int pixelshuffle2d_channeltospace_csleq8(
    const uint n, const uint ic, const uint oc,
    const uint iw, const uint ow,
    const uint ih, const uint oh,
    const uint s,
    ...
    if (cs < AVX1_FLOAT_STRIDE) {
        return pixelshuffle2d_channeltospace_cs2to3(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
    if (cs == AVX1_FLOAT_STRIDE) {
        return pixelshuffle2d_channeltospace_cs4(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
    if (cs < AVX2_FLOAT_STRIDE) {
        return pixelshuffle2d_channeltospace_cs5to7(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
    if (cs == AVX2_FLOAT_STRIDE) {
        return pixelshuffle2d_channeltospace_cs8(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
...
void AvxBlas::PixelShuffle2D::ChannelToSpace(
    ...
    Util::CheckLength(n * ic * iw * ih, xc);
    Util::CheckLength(n * oc * ow * oh, ys);

    Util::CheckDuplicateArray(xc, ys);

    if (s == 1) {
        Elementwise::Copy(n * ic * iw * ih, xc, ys);
        return;
    }
    ...
    if (cs <= AVX2_FLOAT_STRIDE) {
        ...
        ret = pixelshuffle2d_channeltospace_csleq8(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
        ...
        ret = pixelshuffle2d_channeltospace_aligned(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = pixelshuffle2d_channeltospace_unaligned(n, ic, oc, iw, ow, ih, oh, s, x_ptr, y_ptr);
    }
