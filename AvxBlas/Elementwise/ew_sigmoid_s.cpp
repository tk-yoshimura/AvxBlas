...
__forceinline __m256 _mm256_sigmoid_ps(__m256 x) {
...
int ew_sigmoid_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_sigmoid_ps(x0);
        y1 = _mm256_sigmoid_ps(x1);
        y2 = _mm256_sigmoid_ps(x2);
        y3 = _mm256_sigmoid_ps(x3);
...
void AvxBlas::Elementwise::Sigmoid(UInt32 n, Array<float>^ x, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_sigmoid_s(n, x_ptr, y_ptr);
