...
int ew_relu_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_max_pd(x0, zeros);
        y1 = _mm256_max_pd(x1, zeros);
        y2 = _mm256_max_pd(x2, zeros);
        y3 = _mm256_max_pd(x3, zeros);
...
void AvxBlas::Elementwise::Relu(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_relu_d(n, x_ptr, y_ptr);
