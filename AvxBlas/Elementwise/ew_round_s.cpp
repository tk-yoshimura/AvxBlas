...
int ew_round_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_floor_ps(_mm256_add_ps(x0, p5));
        y1 = _mm256_floor_ps(_mm256_add_ps(x1, p5));
        y2 = _mm256_floor_ps(_mm256_add_ps(x2, p5));
        y3 = _mm256_floor_ps(_mm256_add_ps(x3, p5));
...
void AvxBlas::Elementwise::Round(UInt32 n, Array<float>^ x, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_round_s(n, x_ptr, y_ptr);
