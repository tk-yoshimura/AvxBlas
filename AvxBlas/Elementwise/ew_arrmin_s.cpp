...
int ew_min3_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_min_ps(_mm256_min_ps(x01, x02), x03);
        y1 = _mm256_min_ps(_mm256_min_ps(x11, x12), x13);
        y2 = _mm256_min_ps(_mm256_min_ps(x21, x22), x23);
        y3 = _mm256_min_ps(_mm256_min_ps(x31, x32), x33);
...
int ew_min4_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_min_ps(_mm256_min_ps(x01, x02), _mm256_min_ps(x03, x04));
        y1 = _mm256_min_ps(_mm256_min_ps(x11, x12), _mm256_min_ps(x13, x14));
        y2 = _mm256_min_ps(_mm256_min_ps(x21, x22), _mm256_min_ps(x23, x24));
        y3 = _mm256_min_ps(_mm256_min_ps(x31, x32), _mm256_min_ps(x33, x34));
...
void AvxBlas::Elementwise::Minimum(UInt32 n, array<Array<float>^>^ xs, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    if (xs->Length == 3) {
        ...
        int ret = ew_min3_s(n, x1_ptr, x2_ptr, x3_ptr, y_ptr);
    ...
    int r = 0;
    {
        ...
        int ret = ew_min4_s(n, x1_ptr, x2_ptr, x3_ptr, x4_ptr, y_ptr);
        Util::AssertReturnCode(ret);
        r += 4;
    }
    ...
    while (r < xs->Length) {
        if (xs->Length - r >= 3) {
            ...
            int ret = ew_min4_s(n, x1_ptr, x2_ptr, x3_ptr, y_ptr, y_ptr);
            Util::AssertReturnCode(ret);
            r += 3;
            continue;
        }
        if (xs->Length - r >= 2) {
            ...
            int ret = ew_min3_s(n, x1_ptr, x2_ptr, y_ptr, y_ptr);
            Util::AssertReturnCode(ret);
            break;
        }
        if (xs->Length - r >= 1) {
