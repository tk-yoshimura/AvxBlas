...
int ew_clamp_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_min_pd(_mm256_max_pd(x01, x02), x03);
        y1 = _mm256_min_pd(_mm256_max_pd(x11, x12), x13);
        y2 = _mm256_min_pd(_mm256_max_pd(x21, x22), x23);
        y3 = _mm256_min_pd(_mm256_max_pd(x31, x32), x33);
...
void AvxBlas::Elementwise::Clamp(UInt32 n, Array<double>^ x1, Array<double>^ x2, Array<double>^ x3, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
