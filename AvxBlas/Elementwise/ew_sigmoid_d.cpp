...
__forceinline __m256d _mm256_sigmoid_pd(__m256d x) {
    const __m256d bitmask = _mm256_set1_pd(_m64(0x8000000000000000ul).f);
    const __m256d ones = _mm256_set1_pd(1);

    __m256d y = _mm256_div_pd(ones, _mm256_add_pd(ones, _mm256_exp_pd(_mm256_xor_pd(bitmask, x))));
...
int ew_sigmoid_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_sigmoid_pd(x0);
        y1 = _mm256_sigmoid_pd(x1);
        y2 = _mm256_sigmoid_pd(x2);
        y3 = _mm256_sigmoid_pd(x3);
...
void AvxBlas::Elementwise::Sigmoid(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_sigmoid_d(n, x_ptr, y_ptr);
