...
int ew_max3_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_max_pd(_mm256_max_pd(x01, x02), x03);
        y1 = _mm256_max_pd(_mm256_max_pd(x11, x12), x13);
        y2 = _mm256_max_pd(_mm256_max_pd(x21, x22), x23);
        y3 = _mm256_max_pd(_mm256_max_pd(x31, x32), x33);
...
int ew_max4_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_max_pd(_mm256_max_pd(x01, x02), _mm256_max_pd(x03, x04));
        y1 = _mm256_max_pd(_mm256_max_pd(x11, x12), _mm256_max_pd(x13, x14));
        y2 = _mm256_max_pd(_mm256_max_pd(x21, x22), _mm256_max_pd(x23, x24));
        y3 = _mm256_max_pd(_mm256_max_pd(x31, x32), _mm256_max_pd(x33, x34));
...
void AvxBlas::Elementwise::Maximum(UInt32 n, array<Array<double>^>^ xs, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    if (xs->Length <= 2) {
        if (xs->Length == 0) {
            ...
            Elementwise::Maximum(n, xs[0], xs[1], y);
        }
        return;
    }
    ...
    if (xs->Length == 3) {
        ...
        int ret = ew_max3_d(n, x1_ptr, x2_ptr, x3_ptr, y_ptr);
    ...
    int r = 0;
    {
        ...
        int ret = ew_max4_d(n, x1_ptr, x2_ptr, x3_ptr, x4_ptr, y_ptr);
        Util::AssertReturnCode(ret);
        r += 4;
    }
    ...
    while (r < xs->Length) {
        if (xs->Length - r >= 3) {
            ...
            int ret = ew_max4_d(n, x1_ptr, x2_ptr, x3_ptr, y_ptr, y_ptr);
            Util::AssertReturnCode(ret);
            r += 3;
            continue;
        }
        if (xs->Length - r >= 2) {
            ...
            int ret = ew_max3_d(n, x1_ptr, x2_ptr, y_ptr, y_ptr);
            Util::AssertReturnCode(ret);
            break;
        }
        if (xs->Length - r >= 1) {
            Elementwise::Maximum(n, xs[r], y, y);
            break;
        }
    }
}
