...
int ew_atan2_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_atan2_pd(x01, x02);
        y1 = _mm256_atan2_pd(x11, x12);
        y2 = _mm256_atan2_pd(x21, x22);
        y3 = _mm256_atan2_pd(x31, x32);
...
void AvxBlas::Elementwise::Arctan2(UInt32 n, Array<double>^ x1, Array<double>^ x2, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_atan2_d(n, x1_ptr, x2_ptr, y_ptr);
