...
int ew_arccos_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_acos_ps(x0);
        y1 = _mm256_acos_ps(x1);
        y2 = _mm256_acos_ps(x2);
        y3 = _mm256_acos_ps(x3);
...
void AvxBlas::Elementwise::Arccos(UInt32 n, Array<float>^ x, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_arccos_s(n, x_ptr, y_ptr);
