...
int ew_pow_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_pow_ps(x01, x02);
        y1 = _mm256_pow_ps(x11, x12);
        y2 = _mm256_pow_ps(x21, x22);
        y3 = _mm256_pow_ps(x31, x32);
...
void AvxBlas::Elementwise::Pow(UInt32 n, Array<float>^ x1, Array<float>^ x2, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_pow_s(n, x1_ptr, x2_ptr, y_ptr);
