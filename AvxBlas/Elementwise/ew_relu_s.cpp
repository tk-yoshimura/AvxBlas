...
int ew_relu_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_max_ps(x0, zeros);
        y1 = _mm256_max_ps(x1, zeros);
        y2 = _mm256_max_ps(x2, zeros);
        y3 = _mm256_max_ps(x3, zeros);
...
void AvxBlas::Elementwise::Relu(UInt32 n, Array<float>^ x, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_relu_s(n, x_ptr, y_ptr);
