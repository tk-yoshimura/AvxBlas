...
int ew_signedpow_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_signedpow_pd(x01, x02);
        y1 = _mm256_signedpow_pd(x11, x12);
        y2 = _mm256_signedpow_pd(x21, x22);
        y3 = _mm256_signedpow_pd(x31, x32);
...
void AvxBlas::Elementwise::SignedPow(UInt32 n, Array<double>^ x1, Array<double>^ x2, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_signedpow_d(n, x1_ptr, x2_ptr, y_ptr);
