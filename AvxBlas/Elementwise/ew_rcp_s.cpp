...
int ew_rcp_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_div_ps(ones, x0);
        y1 = _mm256_div_ps(ones, x1);
        y2 = _mm256_div_ps(ones, x2);
        y3 = _mm256_div_ps(ones, x3);
...
void AvxBlas::Elementwise::Rcp(UInt32 n, Array<float>^ x, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_rcp_s(n, x_ptr, y_ptr);
