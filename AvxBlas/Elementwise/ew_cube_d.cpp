...
int ew_cube_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_cube_pd(x0);
        y1 = _mm256_cube_pd(x1);
        y2 = _mm256_cube_pd(x2);
        y3 = _mm256_cube_pd(x3);
...
void AvxBlas::Elementwise::Cube(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_cube_d(n, x_ptr, y_ptr);
