...
int ew_log2_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_log2_pd(x0);
        y1 = _mm256_log2_pd(x1);
        y2 = _mm256_log2_pd(x2);
        y3 = _mm256_log2_pd(x3);
...
void AvxBlas::Elementwise::Log2(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_log2_d(n, x_ptr, y_ptr);
