...
int ew_round_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_floor_pd(_mm256_add_pd(x0, p5));
        y1 = _mm256_floor_pd(_mm256_add_pd(x1, p5));
        y2 = _mm256_floor_pd(_mm256_add_pd(x2, p5));
        y3 = _mm256_floor_pd(_mm256_add_pd(x3, p5));
...
void AvxBlas::Elementwise::Round(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_round_d(n, x_ptr, y_ptr);
