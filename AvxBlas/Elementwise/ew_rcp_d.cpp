...
int ew_rcp_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_div_pd(ones, x0);
        y1 = _mm256_div_pd(ones, x1);
        y2 = _mm256_div_pd(ones, x2);
        y3 = _mm256_div_pd(ones, x3);
...
void AvxBlas::Elementwise::Rcp(UInt32 n, Array<double>^ x, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_rcp_d(n, x_ptr, y_ptr);
