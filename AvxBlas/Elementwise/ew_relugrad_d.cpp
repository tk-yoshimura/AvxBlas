...
__forceinline __m256d _mm256_relugrad_pd(__m256d x1, __m256d x2) {
    const __m256d zeros = _mm256_setzero_pd();

    __m256d y = _mm256_and_pd(x1, _mm256_cmp_pd(x2, zeros, _CMP_GT_OS));
...
int ew_relugrad_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_relugrad_pd(x01, x02);
        y1 = _mm256_relugrad_pd(x11, x12);
        y2 = _mm256_relugrad_pd(x21, x22);
        y3 = _mm256_relugrad_pd(x31, x32);
...
void AvxBlas::Elementwise::ReluGrad(UInt32 n, Array<double>^ x1, Array<double>^ x2, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = ew_relugrad_d(n, x1_ptr, x2_ptr, y_ptr);
