...
int pool2d_avgunpool_n32x_seqk_s(
...
int pool2d_avgunpool_n32x_sltk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_n32x_sgtk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_aligned_seqk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_aligned_sltk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_aligned_sgtk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_unaligned_seqk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_unaligned_sltk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
int pool2d_avgunpool_unaligned_sgtk_s(
    const uint n, const uint c,
    const uint iw, const uint ow, const uint sx, const uint kw,
    const uint ih, const uint oh, const uint sy, const uint kh,
    infloats dy_ptr, outfloats dx_ptr) {
...
void AvxBlas::Pool2D::AverageUnpooling(
    ...
    if ((c % (AVX2_FLOAT_STRIDE * 4)) == 0) {
        if (sx == kw && sy == kh) {
            ...
            ret = pool2d_avgunpool_n32x_seqk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx < kw || sy < kh) {
#ifdef _DEBUG
            ...
            ret = pool2d_avgunpool_n32x_sltk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx >= kw && sy >= kh) {
#ifdef _DEBUG
            Console::WriteLine("type n32x sx >= kx and sy >= ky");
            ...
            ret = pool2d_avgunpool_n32x_sgtk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
    }
        ...
        if (sx == kw && sy == kh) {
            ...
            ret = pool2d_avgunpool_aligned_seqk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx < kw || sy < kh) {
#ifdef _DEBUG
            ...
            ret = pool2d_avgunpool_aligned_sltk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx >= kw && sy >= kh) {
#ifdef _DEBUG
            Console::WriteLine("type aligned sx >= kx and sy >= ky");
            ...
            ret = pool2d_avgunpool_aligned_sgtk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
    }
    else {
        if (sx == kw && sy == kh) {
            ...
            ret = pool2d_avgunpool_unaligned_seqk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx < kw || sy < kh) {
#ifdef _DEBUG
            ...
            ret = pool2d_avgunpool_unaligned_sltk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
        else if (sx >= kw && sy >= kh) {
#ifdef _DEBUG
            Console::WriteLine("type unaligned sx >= kx and sy >= ky");
            ...
            ret = pool2d_avgunpool_unaligned_sgtk_s(n, c, iw, ow, sx, kw, ih, oh, sy, kh, dy_ptr, dx_ptr);
        }
    }
    if (ret != SUCCESS) {
    ...
    Constant::Mul(n * c * iw * ih, dx, 1.0f / (kw * kh), dx);
}