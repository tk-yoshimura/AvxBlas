    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_n32x_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix);
            }
        }
...
int pool1d_maxunpool_n32x_sltk_s(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_add_n32x_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix);
            }
        }
...
int pool1d_maxunpool_aligned_seqk_s(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_aligned_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix);
            }
        }
...
int pool1d_maxunpool_aligned_sltk_s(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_add_aligned_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix);
            }
        }
...
int pool1d_maxunpool_unaligned_seqk_s(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_unaligned_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix, mask);
            }
        }
...
int pool1d_maxunpool_unaligned_sltk_s(
    ...
    for (uint i = 0; i < n; i++) {
        for (uint ox = 0, isx = 0; ox < ow; ox++, isx += sx) {
            for (uint kx = 0, ix = isx; kx < kw && ix < iw; kx++, ix++) {
                maxunpooliter_add_unaligned_s(c, x_ptr + c * ix, y_ptr + c * ox, dy_ptr + c * ox, dx_ptr + c * ix, mask);
            }
        }
