...
int const_max_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_max_pd(x0, fillc);
        y1 = _mm256_max_pd(x1, fillc);
        y2 = _mm256_max_pd(x2, fillc);
        y3 = _mm256_max_pd(x3, fillc);
...
void AvxBlas::Constant::Maximum(UInt32 n, Array<double>^ x, double c, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = const_max_d(n, x_ptr, c, y_ptr);
