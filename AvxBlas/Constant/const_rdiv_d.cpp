...
int const_rdiv_d(
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_div_pd(fillc, x0);
        y1 = _mm256_div_pd(fillc, x1);
        y2 = _mm256_div_pd(fillc, x2);
        y3 = _mm256_div_pd(fillc, x3);
...
void AvxBlas::Constant::RDiv(UInt32 n, Array<double>^ x, double c, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = const_rdiv_d(n, x_ptr, c, y_ptr);
