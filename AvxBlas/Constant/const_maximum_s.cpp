...
int const_max_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_max_ps(x0, fillc);
        y1 = _mm256_max_ps(x1, fillc);
        y2 = _mm256_max_ps(x2, fillc);
        y3 = _mm256_max_ps(x3, fillc);
...
void AvxBlas::Constant::Maximum(UInt32 n, Array<float>^ x, float c, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = const_max_s(n, x_ptr, c, y_ptr);
