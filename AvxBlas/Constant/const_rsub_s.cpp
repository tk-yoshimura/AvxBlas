...
int const_rsub_s(
    ...
    while (r >= AVX2_FLOAT_STRIDE * 4) {
        ...
        y0 = _mm256_sub_ps(fillc, x0);
        y1 = _mm256_sub_ps(fillc, x1);
        y2 = _mm256_sub_ps(fillc, x2);
        y3 = _mm256_sub_ps(fillc, x3);
...
void AvxBlas::Constant::RSub(UInt32 n, Array<float>^ x, float c, Array<float>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = const_rsub_s(n, x_ptr, c, y_ptr);
