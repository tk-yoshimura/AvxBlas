...
int const_clamp_d(
    const uint n,
    indoubles x_ptr, const double cmin, const double cmax, outdoubles y_ptr) {
    ...
    const __m256d fillcmin = _mm256_set1_pd(cmin);
    const __m256d fillcmax = _mm256_set1_pd(cmax);
    ...
    while (r >= AVX2_DOUBLE_STRIDE * 4) {
        ...
        y0 = _mm256_min_pd(_mm256_max_pd(x0, fillcmin), fillcmax);
        y1 = _mm256_min_pd(_mm256_max_pd(x1, fillcmin), fillcmax);
        y2 = _mm256_min_pd(_mm256_max_pd(x2, fillcmin), fillcmax);
        y3 = _mm256_min_pd(_mm256_max_pd(x3, fillcmin), fillcmax);
...
void AvxBlas::Constant::Clamp(UInt32 n, Array<double>^ x, double cmin, double cmax, Array<double>^ y) {
    if (n <= 0) {
        return;
    }
    ...
    int ret = const_clamp_d(n, x_ptr, cmin, cmax, y_ptr);
