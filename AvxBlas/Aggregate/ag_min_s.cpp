...
int ag_min_stride1_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            x = _mm256_condmaskload_ps(x_ptr, mask, pinf);
...
int ag_min_stride3_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2;
        __m256 s0 = pinf, s1 = pinf, s2 = pinf;
        ...
        if (((size_t)x_ptr % AVX2_ALIGNMENT) == 0) {
            while (r >= AVX2_FLOAT_STRIDE) {
                ...
                s0 = _mm256_min_ps(x0, s0);
                s1 = _mm256_min_ps(x1, s1);
                s2 = _mm256_min_ps(x2, s2);
        ...
        __m128 y = _mm256_min24to3_ps(s0, s1, s2);
...
int ag_min_stride4_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            x = _mm256_condmaskload_ps(x_ptr, mask4, pinf);
...
int ag_min_stride5_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256 s0 = pinf, s1 = pinf, s2 = pinf, s3 = pinf, s4 = pinf;
        ...
        if (((size_t)x_ptr % AVX2_ALIGNMENT) == 0) {
            while (r >= AVX2_FLOAT_STRIDE) {
                ...
                s1 = _mm256_min_ps(x1, s1);
                s2 = _mm256_min_ps(x2, s2);
                s3 = _mm256_min_ps(x3, s3);
                s4 = _mm256_min_ps(x4, s4);
        ...
        if (r > AVX2_FLOAT_STRIDE * 4 / 5) {
            _mm256_condmaskload_x5_ps(x_ptr, x0, x1, x2, x3, x4, mask, pinf);
        ...
        __m256 y = _mm256_min40to5_ps(s0, s1, s2, s3, s4);
...
int ag_min_stride6_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256 y = _mm256_min24to6_ps(s0, s1, s2);
...
int ag_min_stride25to31_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2, x3;
        __m256 buf0 = pinf, buf1 = pinf, buf2 = pinf, buf3 = pinf;

        for (uint j = 0; j < samples; j++) {
            _mm256_condmaskload_x4_ps(x_ptr, x0, x1, x2, x3, mask, pinf);

            buf0 = _mm256_min_ps(x0, buf0);
            buf1 = _mm256_min_ps(x1, buf1);
            buf2 = _mm256_min_ps(x2, buf2);
            buf3 = _mm256_min_ps(x3, buf3);
...
int ag_min_stride32_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2, x3;
        __m256 s0 = pinf, s1 = pinf, s2 = pinf, s3 = pinf;
...
int ag_min_strideleq8_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {

    if (stride == 1) {
        return ag_min_stride1_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 2) {
        return ag_min_stride2_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 3) {
        return ag_min_stride3_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 4) {
        return ag_min_stride4_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 5) {
        return ag_min_stride5_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 6) {
        return ag_min_stride6_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 7) {
        return ag_min_stride7_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == AVX2_FLOAT_STRIDE) {
        return ag_min_stride8_s(n, samples, stride, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_min_aligned_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {
    ...
    if (stride == AVX2_FLOAT_STRIDE * 2) {
        return ag_min_stride16_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_FLOAT_STRIDE * 3) {
        return ag_min_stride24_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_FLOAT_STRIDE * 4) {
        return ag_min_stride32_s(n, samples, stride, x_ptr, y_ptr);
    }
    ...
    for (uint i = 0; i < n; i++) {
        fill_aligned_s(stride, HUGE_VALF, s_ptr);
...
int ag_min_unaligned_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {

    if (stride <= AVX2_FLOAT_STRIDE) {
        return ag_min_strideleq8_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 2) {
        return ag_min_stride9to15_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 3) {
        return ag_min_stride17to23_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 4) {
        return ag_min_stride25to31_s(n, samples, stride, x_ptr, y_ptr);
    }
    ...
    for (uint i = 0; i < n; i++) {
        fill_aligned_s(ssize, HUGE_VALF, s_ptr);
...
void AvxBlas::Aggregate::Min(UInt32 n, UInt32 samples, UInt32 stride, Array<float>^ x, Array<float>^ y) {
    if (n <= 0 || stride <= 0) {
        return;
    }
    ...
    if ((stride & AVX2_FLOAT_REMAIN_MASK) == 0u) {
        ...
        ret = ag_min_aligned_s(n, samples, stride, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_min_unaligned_s(n, samples, stride, x_ptr, y_ptr);
    }
