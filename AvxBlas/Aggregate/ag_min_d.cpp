...
int ag_min_stride1_d(
    ...
    const __m256d pinf = _mm256_set1_pd(HUGE_VAL);
...
int ag_min_stride3_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256d y = _mm256_min12to3_pd(s0, s1, s2);
...
int ag_min_stride5_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256d s0 = pinf, s1 = pinf, s2 = pinf, s3 = pinf, s4 = pinf;
        ...
        __m256dx2 y = _mm256_min20to5_pd(s0, s1, s2, s3, s4);
...
int ag_min_stride6_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256dx2 y = _mm256_min12to6_pd(s0, s1, s2);
...
int ag_min_strideleq8_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {

    if (stride == 1) {
        return ag_min_stride1_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 2) {
        return ag_min_stride2_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 3) {
        return ag_min_stride3_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 4) {
        return ag_min_stride4_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 5) {
        return ag_min_stride5_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 6) {
        return ag_min_stride6_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 7) {
        return ag_min_stride7_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 8) {
        return ag_min_stride8_d(n, samples, stride, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_min_aligned_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {
    ...
    if (stride == AVX2_DOUBLE_STRIDE * 3) {
        return ag_min_stride12_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_DOUBLE_STRIDE * 4) {
        return ag_min_stride16_d(n, samples, stride, x_ptr, y_ptr);
    }
...
int ag_min_unaligned_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {

    if (stride <= AVX2_DOUBLE_STRIDE * 2) {
        return ag_min_strideleq8_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_DOUBLE_STRIDE * 3) {
        return ag_min_stride9to11_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_DOUBLE_STRIDE * 4) {
        return ag_min_stride13to15_d(n, samples, stride, x_ptr, y_ptr);
    }
...
void AvxBlas::Aggregate::Min(UInt32 n, UInt32 samples, UInt32 stride, Array<double>^ x, Array<double>^ y) {
    ...
    if ((stride & AVX2_DOUBLE_REMAIN_MASK) == 0u) {
        ...
        ret = ag_min_aligned_d(n, samples, stride, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_min_unaligned_d(n, samples, stride, x_ptr, y_ptr);
    }
