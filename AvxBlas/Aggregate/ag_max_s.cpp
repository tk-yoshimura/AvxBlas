...
int ag_max_stride1_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            ...
            x = _mm256_or_ps(x, _mm256_andnot_ps(_mm256_castsi256_ps(mask), minf));
...
int ag_max_stride3_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2;
        __m256 s0 = minf, s1 = minf, s2 = minf;
        ...
        __m128 y = _mm256_max24to3_ps(s0, s1, s2);
...
int ag_max_stride4_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            ...
            x = _mm256_or_ps(x, _mm256_andnot_ps(_mm256_castsi256_ps(mask4), minf));
...
int ag_max_stride5_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256 s0 = minf, s1 = minf, s2 = minf, s3 = minf, s4 = minf;
        ...
        __m256 y = _mm256_max40to5_ps(s0, s1, s2, s3, s4);
...
int ag_max_stride6_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256 y = _mm256_max24to6_ps(s0, s1, s2);
...
int ag_max_stride25to31_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2, x3;
        __m256 buf0 = minf, buf1 = minf, buf2 = minf, buf3 = minf;

        for (uint j = 0; j < samples; j++) {
            _mm256_maskload_x4_ps(x_ptr, x0, x1, x2, x3, mask);
            x3 = _mm256_or_ps(x3, _mm256_andnot_ps(_mm256_castsi256_ps(mask), minf));

            buf0 = _mm256_max_ps(x0, buf0);
            buf1 = _mm256_max_ps(x1, buf1);
            buf2 = _mm256_max_ps(x2, buf2);
            buf3 = _mm256_max_ps(x3, buf3);
...
int ag_max_stride32_s(
    ...
    for (uint i = 0; i < n; i++) {
        __m256 x0, x1, x2, x3;
        __m256 s0 = minf, s1 = minf, s2 = minf, s3 = minf;
...
int ag_max_strideleq8_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {

    if (stride == 1) {
        return ag_max_stride1_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 2) {
        return ag_max_stride2_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 3) {
        return ag_max_stride3_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 4) {
        return ag_max_stride4_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 5) {
        return ag_max_stride5_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 6) {
        return ag_max_stride6_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 7) {
        return ag_max_stride7_s(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == AVX2_FLOAT_STRIDE) {
        return ag_max_stride8_s(n, samples, stride, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_max_aligned_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {
    ...
    if (stride == AVX2_FLOAT_STRIDE * 2) {
        return ag_max_stride16_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_FLOAT_STRIDE * 3) {
        return ag_max_stride24_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_FLOAT_STRIDE * 4) {
        return ag_max_stride32_s(n, samples, stride, x_ptr, y_ptr);
    }
...
int ag_max_unaligned_s(
    const uint n, const uint samples, const uint stride,
    infloats x_ptr, outfloats y_ptr) {

    if (stride <= AVX2_FLOAT_STRIDE) {
        return ag_max_strideleq8_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 2) {
        return ag_max_stride9to15_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 3) {
        return ag_max_stride17to23_s(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_FLOAT_STRIDE * 4) {
        return ag_max_stride25to31_s(n, samples, stride, x_ptr, y_ptr);
    }
...
void AvxBlas::Aggregate::Max(UInt32 n, UInt32 samples, UInt32 stride, Array<float>^ x, Array<float>^ y) {
    ...
    if ((stride & AVX2_FLOAT_REMAIN_MASK) == 0u) {
        ...
        ret = ag_max_aligned_s(n, samples, stride, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_max_unaligned_s(n, samples, stride, x_ptr, y_ptr);
    }
