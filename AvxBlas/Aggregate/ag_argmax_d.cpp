...
int ag_argmax_samples2_d(
    ...
    while (r >= AVX2_EPI32_STRIDE) {
        ...
        s0 = _mm256_maxwise2_pd(x0);
        s1 = _mm256_maxwise2_pd(x1);
        s2 = _mm256_maxwise2_pd(x2);
        s3 = _mm256_maxwise2_pd(x3);
...
int ag_argmax_samples3_d(
    ...
    while (r >= AVX2_EPI32_STRIDE / 2) {
        ...
        s0 = _mm256_maxwise3_pd(x0);
        s1 = _mm256_maxwise3_pd(x1);
        s2 = _mm256_maxwise3_pd(x2);
        s3 = _mm256_maxwise3_pd(x3);
...
int ag_argmax_samples4_d(
    ...
    while (r >= AVX2_EPI32_STRIDE / 2) {
        ...
        s0 = _mm256_maxwise4_pd(x0);
        s1 = _mm256_maxwise4_pd(x1);
        s2 = _mm256_maxwise4_pd(x2);
        s3 = _mm256_maxwise4_pd(x3);
...
int ag_argmax_samples5to7_d(
    ...
    const __m256d minf = _mm256_set1_pd(-HUGE_VALF);
    ...
    while (r >= AVX2_EPI32_STRIDE / 2) {
        ...
        s45 = _mm256_max_pd(_mm256_maxwise4_pd(x4), _mm256_maxwise4_pd(x5));
        s67 = _mm256_max_pd(_mm256_maxwise4_pd(x6), _mm256_maxwise4_pd(x7));
...
int ag_argmax_samplesleq8_d(
    const uint n, const uint samples,
    indoubles x_ptr, outuints y_ptr) {

    if (samples == 2) {
        return ag_argmax_samples2_d(n, samples, x_ptr, y_ptr);
    }
    else if (samples == 3) {
        return ag_argmax_samples3_d(n, samples, x_ptr, y_ptr);
    }
    else if (samples == 4) {
        return ag_argmax_samples4_d(n, samples, x_ptr, y_ptr);
    }
    else if (samples < AVX2_DOUBLE_STRIDE * 2) {
        return ag_argmax_samples5to7_d(n, samples, x_ptr, y_ptr);
    }
    else if (samples == AVX2_DOUBLE_STRIDE * 2) {
        return ag_argmax_samples8_d(n, samples, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_argmax_samples_aligned_d(
    const uint n, const uint samples,
    indoubles x_ptr, outuints y_ptr) {
    ...
    if (samples == AVX2_DOUBLE_STRIDE * 3) {
        return ag_argmax_samples12_d(n, samples, x_ptr, y_ptr);
    }
    if (samples == AVX2_DOUBLE_STRIDE * 4) {
        return ag_argmax_samples16_d(n, samples, x_ptr, y_ptr);
    }

    return ag_argmax_aligned_d(n, samples, x_ptr, y_ptr);
}

int ag_argmax_samples_unaligned_d(
    const uint n, const uint samples,
    indoubles x_ptr, outuints y_ptr) {

    if (samples <= AVX2_DOUBLE_STRIDE * 2) {
        return ag_argmax_samplesleq8_d(n, samples, x_ptr, y_ptr);
    }
    if (samples <= AVX2_DOUBLE_STRIDE * 3) {
        return ag_argmax_samples9to11_d(n, samples, x_ptr, y_ptr);
    }
    if (samples <= AVX2_DOUBLE_STRIDE * 4) {
        return ag_argmax_samples13to15_d(n, samples, x_ptr, y_ptr);
    }

    return ag_argmax_unaligned_d(n, samples, x_ptr, y_ptr);
}

#pragma managed

void AvxBlas::Aggregate::ArgMax(UInt32 n, UInt32 samples, UInt32 stride, Array<double>^ x, Array<Int32>^ y) {
    if (n <= 0 || stride <= 0) {
        return;
    }
    ...
    if ((samples & AVX2_DOUBLE_REMAIN_MASK) == 0u) {
        ...
        ret = ag_argmax_samples_aligned_d(n * stride, samples, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_argmax_samples_unaligned_d(n * stride, samples, x_ptr, y_ptr);
    }
