...
int ag_prod_stride1_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            x = _mm256_condmaskload_ps(x_ptr, mask, ones);
...
int ag_prod_stride3_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > AVX2_FLOAT_STRIDE * 2 / 3) {
            _mm256_condmaskload_x3_ps(x_ptr, x0, x1, x2, mask, ones);
            ...
            _mm256_condmaskload_x2_ps(x_ptr, x0, x1, mask, ones);

            s0 = _mm256_mul_ps(x0, s0);
            s1 = _mm256_mul_ps(x1, s1);
        }
        else if (r > 0) {
            _mm256_condmaskload_x1_ps(x_ptr, x0, mask, ones);
...
int ag_prod_stride4_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            x = _mm256_condmaskload_ps(x_ptr, mask4, ones);
...
int ag_prod_stride5_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > AVX2_FLOAT_STRIDE * 4 / 5) {
            _mm256_condmaskload_x5_ps(x_ptr, x0, x1, x2, x3, x4, mask, ones);
            ...
            _mm256_condmaskload_x4_ps(x_ptr, x0, x1, x2, x3, mask, ones);
