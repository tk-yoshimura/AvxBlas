...
#include "../constants.h"
#include "../utils.h"
#include "../Inline/inline_prod_d.hpp"
...
int ag_prod_stride1_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r > 0) {
            ...
            x = _mm256_or_pd(x, _mm256_andnot_pd(_mm256_castsi256_pd(mask), ones));
...
int ag_prod_stride2_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m128d y = _mm256_prod4to2_pd(s);
...
int ag_prod_stride3_d(
    ...
    for (uint i = 0; i < n; i++) {
        __m256d x0, x1, x2;
        __m256d s0 = ones, s1 = ones, s2 = ones;
        ...
        if (((size_t)x_ptr % AVX2_ALIGNMENT) == 0) {
            while (r >= AVX2_DOUBLE_STRIDE) {
                ...
                s0 = _mm256_mul_pd(x0, s0);
                s1 = _mm256_mul_pd(x1, s1);
                s2 = _mm256_mul_pd(x2, s2);
        ...
        if (r > AVX2_DOUBLE_STRIDE * 2 / 3) {
            ...
            x2 = _mm256_or_pd(x2, _mm256_andnot_pd(_mm256_castsi256_pd(mask), ones));
            ...
            x1 = _mm256_or_pd(x1, _mm256_andnot_pd(_mm256_castsi256_pd(mask), ones));
            ...
            x0 = _mm256_or_pd(x0, _mm256_andnot_pd(_mm256_castsi256_pd(mask), ones));

            s0 = _mm256_mul_pd(x0, s0);
        }

        __m256d y = _mm256_prod12to3_pd(s0, s1, s2);
...
int ag_prod_stride5_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256d s0 = ones, s1 = ones, s2 = ones, s3 = ones, s4 = ones;
        ...
        if (((size_t)x_ptr % AVX2_ALIGNMENT) == 0) {
            while (r >= AVX2_DOUBLE_STRIDE) {
                ...
                s1 = _mm256_mul_pd(x1, s1);
                s2 = _mm256_mul_pd(x2, s2);
                s3 = _mm256_mul_pd(x3, s3);
                s4 = _mm256_mul_pd(x4, s4);
        ...
        if (r > AVX2_DOUBLE_STRIDE * 3 / 5) {
            ...
            x3 = _mm256_or_pd(x3, _mm256_andnot_pd(_mm256_castsi256_pd(mask), ones));
        ...
        __m256dx2 y = _mm256_prod20to5_pd(s0, s1, s2, s3, s4);
...
int ag_prod_stride6_d(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        __m256dx2 y = _mm256_prod12to6_pd(s0, s1, s2);
...
int ag_prod_stride7_d(
    ...
    for (uint i = 0; i < n; i++) {
        __m256d x0, x1;
        __m256d s0 = ones, s1 = ones;

        for (uint j = 0; j < samples; j++) {
            _mm256_maskload_x2_pd(x_ptr, x0, x1, mask3);
            x1 = _mm256_or_pd(x1, _mm256_andnot_pd(_mm256_castsi256_pd(mask3), ones));
...
int ag_prod_stride13to15_d(
    ...
    for (uint i = 0; i < n; i++) {
        __m256d x0, x1, x2, x3;
        __m256d s0 = ones, s1 = ones, s2 = ones, s3 = ones;
...
int ag_prod_strideleq8_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {

    if (stride == 1) {
        return ag_prod_stride1_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 2) {
        return ag_prod_stride2_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 3) {
        return ag_prod_stride3_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 4) {
        return ag_prod_stride4_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 5) {
        return ag_prod_stride5_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 6) {
        return ag_prod_stride6_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 7) {
        return ag_prod_stride7_d(n, samples, stride, x_ptr, y_ptr);
    }
    else if (stride == 8) {
        return ag_prod_stride8_d(n, samples, stride, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_prod_aligned_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {
    ...
    if (stride == AVX2_DOUBLE_STRIDE * 3) {
        return ag_prod_stride12_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride == AVX2_DOUBLE_STRIDE * 4) {
        return ag_prod_stride16_d(n, samples, stride, x_ptr, y_ptr);
    }
...
int ag_prod_unaligned_d(
    const uint n, const uint samples, const uint stride,
    indoubles x_ptr, outdoubles y_ptr) {

    if (stride <= AVX2_DOUBLE_STRIDE * 2) {
        return ag_prod_strideleq8_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_DOUBLE_STRIDE * 3) {
        return ag_prod_stride9to11_d(n, samples, stride, x_ptr, y_ptr);
    }
    if (stride <= AVX2_DOUBLE_STRIDE * 4) {
        return ag_prod_stride13to15_d(n, samples, stride, x_ptr, y_ptr);
    }
...
void AvxBlas::Aggregate::Prod(UInt32 n, UInt32 samples, UInt32 stride, Array<double>^ x, Array<double>^ y) {
    ...
    if ((stride & AVX2_DOUBLE_REMAIN_MASK) == 0u) {
        ...
        ret = ag_prod_aligned_d(n, samples, stride, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_prod_unaligned_d(n, samples, stride, x_ptr, y_ptr);
    }
