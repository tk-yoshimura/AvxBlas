...
int ag_argmax_samples2_s(
    ...
    while (r >= AVX2_EPI32_STRIDE * 2) {
        ...
        s0 = _mm256_maxwise2_ps(x0);
        s1 = _mm256_maxwise2_ps(x1);
        s2 = _mm256_maxwise2_ps(x2);
        s3 = _mm256_maxwise2_ps(x3);
...
int ag_argmax_samples3_s(
    ...
    while (r >= AVX2_EPI32_STRIDE) {
        ...
        s0 = _mm256_maxwise3_ps(x0);
        s1 = _mm256_maxwise3_ps(x1);
        s2 = _mm256_maxwise3_ps(x2);
        s3 = _mm256_maxwise3_ps(x3);
...
int ag_argmax_samples4_s(
    ...
    while (r >= AVX2_EPI32_STRIDE) {
        ...
        s0 = _mm256_maxwise4_ps(x0);
        s1 = _mm256_maxwise4_ps(x1);
        s2 = _mm256_maxwise4_ps(x2);
        s3 = _mm256_maxwise4_ps(x3);
...
int ag_argmax_samples5to7_s(
    ...
    while (r >= AVX2_EPI32_STRIDE) {
        ...
        x1 = _mm256_condmaskload_ps(x_ptr + samples, mask, minf);
        x2 = _mm256_condmaskload_ps(x_ptr + samples * 2, mask, minf);
        x3 = _mm256_condmaskload_ps(x_ptr + samples * 3, mask, minf);
        x4 = _mm256_condmaskload_ps(x_ptr + samples * 4, mask, minf);
        x5 = _mm256_condmaskload_ps(x_ptr + samples * 5, mask, minf);
        x6 = _mm256_condmaskload_ps(x_ptr + samples * 6, mask, minf);
        x7 = _mm256_condmaskload_ps(x_ptr + samples * 7, mask, minf);

        s0 = _mm256_maxwise8_ps(x0);
        s1 = _mm256_maxwise8_ps(x1);
        s2 = _mm256_maxwise8_ps(x2);
        s3 = _mm256_maxwise8_ps(x3);
        s4 = _mm256_maxwise8_ps(x4);
        s5 = _mm256_maxwise8_ps(x5);
        s6 = _mm256_maxwise8_ps(x6);
        s7 = _mm256_maxwise8_ps(x7);
...
int ag_argmax_samples9to15_s(
    ...
    while (r >= AVX2_EPI32_STRIDE / 2) {
        ...
        _mm256_condmaskload_x2_ps(x_ptr + samples, x2, x3, mask, minf);
        _mm256_condmaskload_x2_ps(x_ptr + samples * 2, x4, x5, mask, minf);
        _mm256_condmaskload_x2_ps(x_ptr + samples * 3, x6, x7, mask, minf);

        s01 = _mm256_max_ps(_mm256_maxwise8_ps(x0), _mm256_maxwise8_ps(x1));
        s23 = _mm256_max_ps(_mm256_maxwise8_ps(x2), _mm256_maxwise8_ps(x3));
        s45 = _mm256_max_ps(_mm256_maxwise8_ps(x4), _mm256_maxwise8_ps(x5));
        s67 = _mm256_max_ps(_mm256_maxwise8_ps(x6), _mm256_maxwise8_ps(x7));
    ...
    if (r >= AVX2_EPI32_STRIDE / 4) {
        ...
        x1 = _mm256_where_ps(mask, x1, minf);
        x3 = _mm256_where_ps(mask, x3, minf);
...
int ag_argmax_samples17to23_s(
    ...
    while (r >= 2) {
        ...
        _mm256_condmaskload_x3_ps(x_ptr + samples, x3, x4, x5, mask, minf);

        s012 = _mm256_max_ps(_mm256_max_ps(_mm256_maxwise8_ps(x0), _mm256_maxwise8_ps(x1)), _mm256_maxwise8_ps(x2));
        s345 = _mm256_max_ps(_mm256_max_ps(_mm256_maxwise8_ps(x3), _mm256_maxwise8_ps(x4)), _mm256_maxwise8_ps(x5));
...
int ag_argmax_unaligned_s(
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (r >= AVX2_FLOAT_STRIDE) {
            ...
            t = _mm256_max_ps(_mm256_maxwise8_ps(x0), s);
        ...
        if (r > 0) {
            _mm256_condmaskload_x1_ps(x_ptr, x0, mask, minf);
...
int ag_argmax_samplesleq8_s(
    const uint n, const uint samples,
    infloats x_ptr, outuints y_ptr) {

    if (samples == 2) {
        return ag_argmax_samples2_s(n, samples, x_ptr, y_ptr);
    }
    else if (samples == 3) {
        return ag_argmax_samples3_s(n, samples, x_ptr, y_ptr);
    }
    else if (samples == 4) {
        return ag_argmax_samples4_s(n, samples, x_ptr, y_ptr);
    }
    else if (samples < AVX2_FLOAT_STRIDE) {
        return ag_argmax_samples5to7_s(n, samples, x_ptr, y_ptr);
    }
    else if (samples == AVX2_FLOAT_STRIDE) {
        return ag_argmax_samples8_s(n, samples, x_ptr, y_ptr);
    }

    return FAILURE_BADPARAM;
}

int ag_argmax_samples_aligned_s(
    const uint n, const uint samples,
    infloats x_ptr, outuints y_ptr) {
    ...
    if (samples == AVX2_FLOAT_STRIDE * 2) {
        return ag_argmax_samples16_s(n, samples, x_ptr, y_ptr);
    }
    if (samples == AVX2_FLOAT_STRIDE * 3) {
        return ag_argmax_samples24_s(n, samples, x_ptr, y_ptr);
    }
    if (samples == AVX2_FLOAT_STRIDE * 4) {
        return ag_argmax_samples32_s(n, samples, x_ptr, y_ptr);
    }

    return ag_argmax_aligned_s(n, samples, x_ptr, y_ptr);
}

int ag_argmax_samples_unaligned_s(
    const uint n, const uint samples,
    infloats x_ptr, outuints y_ptr) {

    if (samples <= AVX2_FLOAT_STRIDE) {
        return ag_argmax_samplesleq8_s(n, samples, x_ptr, y_ptr);
    }
    if (samples <= AVX2_FLOAT_STRIDE * 2) {
        return ag_argmax_samples9to15_s(n, samples, x_ptr, y_ptr);
    }
    if (samples <= AVX2_FLOAT_STRIDE * 3) {
        return ag_argmax_samples17to23_s(n, samples, x_ptr, y_ptr);
    }
    if (samples <= AVX2_FLOAT_STRIDE * 4) {
        return ag_argmax_samples25to31_s(n, samples, x_ptr, y_ptr);
    }

    return ag_argmax_unaligned_s(n, samples, x_ptr, y_ptr);
}

#pragma managed

void AvxBlas::Aggregate::ArgMax(UInt32 n, UInt32 samples, UInt32 stride, Array<float>^ x, Array<Int32>^ y) {
    if (n <= 0 || stride <= 0) {
        return;
    }
    ...
    if ((samples & AVX2_FLOAT_REMAIN_MASK) == 0u) {
        ...
        ret = ag_argmax_samples_aligned_s(n * stride, samples, x_ptr, y_ptr);
    }
    else {
#ifdef _DEBUG
        ...
        ret = ag_argmax_samples_unaligned_s(n * stride, samples, x_ptr, y_ptr);
    }
