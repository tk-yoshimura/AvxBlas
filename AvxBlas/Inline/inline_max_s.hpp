...
__forceinline float _mm_max4to1_ps(__m128 x) {
    __m128 y = _mm_max_ps(x, _mm_movehl_ps(x, x));
    float ret = _mm_cvtss_f32(_mm_max_ss(y, _mm_shuffle_ps(y, y, 1)));

    return ret;
}

// e0,e1,e2,e3,e4,e5,e6,e7 -> max(e0,e1,e2,e3,e4,e5,e6,e7)
__forceinline float _mm256_max8to1_ps(__m256 x) {
    __m128 y = _mm_max_ps(_mm256_castps256_ps128(x), _mm256_extractf128_ps(x, 1));
    __m128 z = _mm_max_ps(y, _mm_movehl_ps(y, y));
    float ret = _mm_cvtss_f32(_mm_max_ss(z, _mm_shuffle_ps(z, z, 1)));

    return ret;
}

// e0,e1,e2,e3,e4,e5,e6,e7 -> max(e0,e2),max(e4,e6),max(e1,e3),max(e5,e7)
__forceinline __m128 _mm256_max8to2_ps(__m256 x) {
    __m128 y = _mm_max_ps(_mm256_castps256_ps128(x), _mm256_extractf128_ps(x, 1));
    __m128 ret = _mm_max_ps(y, _mm_movehl_ps(y, y));

    return ret;
}

// e0,e1,e2,e3,e4,e5,-,- -> max(e0,e3),max(e1,e4),max(e2,e5),_
__forceinline __m128 _mm256_max6to3_ps(__m256 x) {
    const __m256i _perm43 = _mm256_setr_epi32(0, 1, 2, 6, 3, 4, 5, 7);

    __m256 y = _mm256_permutevar8x32_ps(x, _perm43);
    __m128 ret = _mm_max_ps(_mm256_castps256_ps128(y), _mm256_extractf128_ps(y, 1));

    return ret;
}

// e0,e1,e2,e3,e4,e5,e6,e7 -> max(e0,e4),max(e1,e5),max(e2,e6),max(e3,e7)
__forceinline __m128 _mm256_max8to4_ps(__m256 x) {
    __m128 ret = _mm_max_ps(_mm256_castps256_ps128(x), _mm256_extractf128_ps(x, 1));

    return ret;
}

// e0,...,e15 -> max(e0,...,e15)
__forceinline float _mm256_max16to1_ps(__m256 x, __m256 y) {
    float ret = _mm256_max8to1_ps(_mm256_max_ps(x, y));

    return ret;
}

// e0,...,e23 -> max(e0,...,e23)
__forceinline float _mm256_max24to1_ps(__m256 x, __m256 y, __m256 z) {
    float ret = _mm256_max8to1_ps(_mm256_max_ps(_mm256_max_ps(x, y), z));

    return ret;
}

// e0,...,e31 -> max(e0,...,e31)
__forceinline float _mm256_max32to1_ps(__m256 x, __m256 y, __m256 z, __m256 w) {
    float ret = _mm256_max8to1_ps(_mm256_max_ps(_mm256_max_ps(x, y), _mm256_max_ps(z, w)));

    return ret;
}

// e0,...,e23 -> max(e0,e3,...,e21),max(e1,e4,...,e22),max(e2,e5,...,e23),_
__forceinline __m128 _mm256_max24to3_ps(__m256 x0, __m256 x1, __m256 x2) {
    ...
    __m256 w0 = _mm256_max_ps(y0, _mm256_max_ps(y1, y2));
    __m256 w1 = _mm256_max_ps(z0, _mm256_max_ps(z1, z2));

    __m256 s = _mm256_max_ps(w0, w1);

    __m128 ret = _mm_max_ps(_mm256_castps256_ps128(s), _mm256_extractf128_ps(s, 1));

    return ret;
}

// e0,...,e39 -> max(e0,e5,...,e35),max(e1,e6,...,e36),max(e2,e7,...,e37),max(e3,e8,...,e38),max(e4,e9,...,e39),_,_,_
__forceinline __m256 _mm256_max40to5_ps(__m256 x0, __m256 x1, __m256 x2, __m256 x3, __m256 x4) {
    ...
    __m256 w0 = _mm256_max_ps(y0, _mm256_max_ps(_mm256_max_ps(y1, y2), _mm256_max_ps(y3, y4)));
    __m256 w1 = _mm256_max_ps(z0, _mm256_max_ps(_mm256_max_ps(z1, z2), _mm256_max_ps(z3, z4)));
...
// e0,...,e23 -> max(e0,e6,...,e18),max(e1,e7,...,e19),max(e2,e8,...,e20),max(e3,e9,...,e21),max(e4,e10,...,e22),max(e5,e11,...,e23),_,_
__forceinline __m256 _mm256_max24to6_ps(__m256 x0, __m256 x1, __m256 x2) {
